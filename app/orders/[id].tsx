...
export default function OrderDetailScreen() {
  ...
  const paymentSessions = order.paymentSessions?.length
    ...
    : order.paymentSession
      ? [order.paymentSession]
      : [];
  const gallery = order.orderLinks
    .flatMap((link) => [link.purchaseImageUrl, link.warehouseImageUrl, link.warehouseCheckImageUrl])
      ...
      <Section title={`Sản phẩm (${order.orderLinks.length})`} icon="shopping-bag">
        {order.orderLinks.map((link) => (
          <AppCard key={link.orderLinkId} style={styles.productCard}>
            ...
            {link.purchaseImageUrl || link.warehouseImageUrl ? (
              <Image source={{ uri: link.warehouseImageUrl || link.purchaseImageUrl || '' }} style={styles.productImage} />
            ) : null}
